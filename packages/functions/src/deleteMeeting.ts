...
export const handler: APIGatewayProxyHandler = async (event: APIGatewayEvent) => {
  try {
    ...
    logger.info(`Processing request to delete meeting with meetingId: ${meetingId}`);
  
    const chime = new ChimeService();
    const data = await chime.deleteMeeting(meetingId);
    return {
    ...
    logger.error('Error while deleting meeting', error);
    return {
      headers: { 'Content-Type': 'application/json', },
